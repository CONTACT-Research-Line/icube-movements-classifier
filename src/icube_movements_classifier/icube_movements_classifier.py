    ...
    def compute_angles(self, quaternions_old, quaternions, quanti_trial):
        ...
        for trial in range(1,quanti_trial+1):
            ...
            for i in range(1,2):
                ...
                """
                angle_X_rad = math.atan2 (2 * (qd.w * qd.x + qd.y * qd.z), 1 - 2 * (qd.x**2 + qd.y**2))
                angle_Y_rad = math.asin ( 2 * (qd.w * qd.y - qd.z * qd.x) )
                angle_Z_rad = math.atan2( 2 * (qd.w * qd.z + qd.x * qd.y), 1 - 2 * (qd.y**2 + qd.z**2) )
                angle_X = math.degrees(angle_X_rad)
                angle_Y = math.degrees(angle_Y_rad)
                angle_Z = math.degrees(angle_Z_rad)
