...
class SwipeDetector(BaseEventHandler):
    """
    ...
    def __init__(self,
        ...
        if detect_on_face is not None:
            assert (0 <= int(detect_on_face) <= 5,
                    f"[SWIPE DETECTOR] detect_on_face must be within 0 and 5, provided {detect_on_face}")
    ...
    def __touches_to_sequences(self, touches):
        try:
            [self.sequences_by_face[f].extend([(x, y)
                                               ...
                                               for x in range(N_COLS) if
                                               touches[f] is not None and len(touches[f]) > 0 and touches[f][
                                                   N_ROWS * x + y] == '1' and (x, y) not in self.sequences_by_face[f]
        ...
        try:
            if self.face_to_detect_on >= 0:
                if touches[self.face_to_detect_on] == FULLY_COVERED_FACE:
                    self.fire(SwipeEvents.COVER, self.face_to_detect_on)
                    ...
                    if (time.time() - self.long_press_last_time) >= self.long_press_duration:
                        if self.long_press_accumulator[self.face_to_detect_on].count(SwipeEvents.COVER) == len(
                                self.long_press_accumulator[self.face_to_detect_on]):
                            self.fire(SwipeEvents.LONG_PRESS, self.face_to_detect_on)
                        self.long_press_accumulator[self.face_to_detect_on] = []
                ...
                for face_id, face_touch in enumerate(touches):
                    if face_touch == FULLY_COVERED_FACE:
                        self.fire(SwipeEvents.COVER, face_id)
            ...
            try:
                if self.face_to_detect_on >= 0:
                    ...
                    if event is not None and event in self.mapping_event_to_callback:
                        self.fire(event, self.face_to_detect_on)
